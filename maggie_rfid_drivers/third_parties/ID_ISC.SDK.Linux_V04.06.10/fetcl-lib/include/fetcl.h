...
Copyright (C) 2005-2012	FEIG ELECTRONIC GmbH, All Rights Reserved.
						Lange Strasse 4
						D-35781 Weilburg
...
Autor         	:		M. Hultsch
Version       	:		02.02.01 / 13.02.2014 / M. Hultsch + A. Rauch
...
*/
	...
	#endif
	#define FETCL_EXT_CALL __stdcall
	#define FETCL_CALL __cdecl
#else
...
{									//   message-handle or callback
	...
	unsigned int	uiFlag;		// specifies the use of the union (e.g. FETCL_WND_HWND)
	union
	{
...
#endif
		void	(FETCL_CALL* cbFct)(int, int, int);                        // for callback-function
		void	(FETCL_CALL* cbFct2)(void*, int, int, int);                // for callback-function
...
//int DLL_EXT_FUNC FETCL_SetLogPara( int iTransponderHnd, char* cPara, char* cValue );

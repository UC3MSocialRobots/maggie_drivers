...
Copyright (C) 1998-2014	FEIG ELECTRONIC GmbH, All Rights Reserved.
						Lange Strasse 4
						D-35781 Weilburg
...
Version       		:	07.03.00 / 22.05.2014 / M. Hultsch

Operation Systems	:	Windows XP / Vista / 7 / 8
						WindowsCE
						Linux / uClinux / VxWorks
...
#define _FEISC_INCLUDE_H
	...
	#define FEISC_EXT_CALL __stdcall
	#define FEISC_CALL __cdecl
#else
	#define	DLL_EXT_FUNC
	#define FEISC_EXT_CALL
	#define FEISC_CALL
#endif
...
#define FEISC_TABLE_DATA_EXCHANGE		0x1E
...
#define	FEISC_ERR_TASK_PORT_ALREADY_IN_USE	-4086
...
typedef struct _FEISC_EVENT_INIT
{
	...
	union
	{
		...
		void	(CALLBACK *cbFct2)(BSTR, int, int);	// for callback2-function
#endif
		void	(FEISC_CALL *cbFct) (	int iMessage,		// [in] to int casted pointer to message
										int iStatus );		// [in] status byte (>=0) or error code (<0)
		
		void	(FEISC_CALL *cbFct4)(	void* pAny,			// [in] pointer to anything (from struct _FEISC_EVENT_INIT)
										const char* cMsg,	// [in] pointer to message
										int iStatus );		// [in] status byte (>=0) or error code (<0)
		
		void	(FEISC_CALL *cbFct5)(	void* pAny,			// [in] pointer to anything (from struct _FEISC_EVENT_INIT)
										const char* cMsg,	// [in] pointer to message
										int iStatus,		// [in] status byte (>=0) or error code (<0)
										int iEventID );		// [in] Event-ID (see uiUse)
#ifdef __cplusplus
	};
#else
	}Method;
...
typedef struct _FEISC_TASK_INIT
{
	...
	// only for notification and max event tasks
	...
	union
	{
		...
		void (FEISC_CALL *cbFct1)(	void* pAny,					// [in] pointer to anything (from struct _FEISC_TASK_INIT)
		...
		void (FEISC_CALL *cbFct2)(	void* pAny,					// [in] pointer to anything (from struct _FEISC_TASK_INIT)
									int iReaderHnd,				// [in] reader handle of FEISC
									int iTaskID,				// [in] task identifier from FEISC_StartAsyncTask(..)
									int iError,					// [in] OK (=0), error code (<0) or status byte from reader (>0)
									unsigned char ucCmd,		// [in]	reader command
		...
		int	(FEISC_CALL *cbFct3)(	void* pAny,					// [in] pointer to anything (from struct _FEISC_TASK_INIT)
									int iReaderHnd,				// [in] reader handle of FEISC
									int iTaskID,				// [in] task identifier from FEISC_StartAsyncTask(..)
									int iError,					// [in] OK (=0), error code (<0) or status byte from reader (>0)
									unsigned char ucCmd,		// [in]	reader command

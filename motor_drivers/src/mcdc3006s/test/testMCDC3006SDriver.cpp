/**
 * \file: testMCDC3006SDriver.cpp
 *
 *  \date 29/05/2010
 *  \author Victor Gonzalez-Pacheco (mailto: vgonzale@ing.uc3m.es)
 *
 *  Program to test the driver MCDC3006S
 *
 */
...
//void printLimits (MCDC3006SDriver * driver);
//void printMotorSensor (MCDC3006SDriver * driver);
//void printStatus (MCDC3006SDriver * driver);
//void printConfiguration (MCDC3006SDriver * driver);
...
void printMenu()
{
    ...
    cout << endl
         << "--- CONNECT / DISCONNECT ---"
         << endl;
    cout << "[co], [con], [connect] : Connect the driver with the PC"
         << endl;
    cout << "[dis], [disconnect] : Disconnect the driver from the PC"
         << endl;
    // Limits
    cout << endl
         << "--- LIMITS ---"
         << endl;
    cout << "[mp], [maxpos] : Set Maximum Position"
         << endl;
    cout << "[mip], [minpos] : Set Minimum Position"
         << endl;
    cout << "[mv], [maxvel] : Set Maximum Velocity"
         << endl;
    cout << "[ma], [maxacc] : Set Maximum Acceleration"
         << endl;
    cout << "[md], [maxdec] : Set Maximum Deceleration"
         << endl;
    cout << "[clc],[curlimc] : Set Continuous Current Limit"
         << endl;
    cout << "[clp],[curlimp] : Set Peak Current Limit"
         << endl;
    // Movements
    cout << endl
         << "--- MOVEMENTS ---"
         << endl;
    cout << "[mabs],[abs] : Move to an Absolute Position"
         << endl;
    cout << "[mrel],[rel] : Move to a Relative Position"
         << endl;
    cout << "[v],[vel] : Move by velocity"
         << endl;
    // Print Data
    cout << endl
         << "--- PRINT DATA ---"
         << endl;
    cout << "[pl],[plim], [plimit] : Print all limits"
         << endl;
    cout << "[ms],[sens], [sensor] : Print Motor Sensor Data"
         << endl;
    cout << "[ps],[stat], [pstatus] : Print Status"
         << endl;
    cout << "[pc],[pconf] : Print the Driver Configuration"
         << endl;
    cout << "[pa],[pall], [print] : Print all Configuration Data: Limits, Motor Sensor, Status and Configuration"
         << endl;
    // Calibration and power
    cout << endl
         << "--- CALIBRATION & POWER ---"
         << endl;
    cout << "[c],[cal], [calibrate] : Calibrate the Driver"
         << endl;
    cout << "[e],[en], [enable] : Enable the Driver"
         << endl;
    cout << "[d],[di], [disable] : Disable the Driver"
         << endl;
    // Configuration
    cout << endl
         << "--- SAVE & LOAD DRIVER CONFIGURATION ---"
         << endl;
    cout << "[l], [load] : Load configuration from file"
         << endl;
    cout << "[s], [save] : Save configuration to a file"
         << endl;
    cout << "[do], [download] : Download configuration to the driver"
         << endl;
    // Program section
    cout << endl
         << "--- OTHER ---"
...
void menu()
{
    ...
    string defaultConfigFile = "mcdc3006s/configMCDC3006S.xml";
    MCDC3006SDriver driver;
    ...
    do {
        ...
        if (option.compare("h") == 0 || option.compare("help") == 0) {
        ...
        else if (option.compare("ma") == 0 || option.compare("maxacc") == 0) {
            cout << "Maximum acceleration (in rev/sec^2): ";
            cin >> keyInput;
            driver.setMaxAcceleration(atol(keyInput.data()));

            // set max decceleration
        }
        else if (option.compare("md") == 0 || option.compare("maxdec") == 0) {
            cout << "Minimum Position (in rev/sec^2): ";
            cin >> keyInput;
            driver.setMaxDecceleration(atol(keyInput.data()));

            // set continuous current limit
        }
        else if (option.compare("clc") == 0 || option.compare("curlimc") == 0) {
            cout << "Continuous Current Limit (in mA): ";
            cin >> keyInput;
            driver.setContinousCurrentLimit(atoi(keyInput.data()));

            // set peak current limit
        }
        else if (option.compare("clp") == 0 || option.compare("curlimp") == 0) {
            cout << "Peak Current Limit (in mA): ";
            cin >> keyInput;
            driver.setPeakCurrentLimit(atol(keyInput.data()));
            ...
            cout << "Disabling Driver"
                 << endl;
            driver.disable();
            cout << "OK, Driver Disabled"
                 << endl;
            ...
            cin >> lFile;
            cout << "Loading configuration from the file."
                 << endl;
            if (driver.loadConfigFromFile(lFile) != ERR_NOERR) {
                perror("There has been an error loading the configuration.");
            }
            else {
                cout << "Configuration loaded. Now you can download it to the driver"
                     << endl;
            }
            ...
            cout << "Unknown command"
                 << endl;
            cout << "Press \"h\" to print all the supported commands"
                 << endl;
            cout << " Option: ";
        }
    }
